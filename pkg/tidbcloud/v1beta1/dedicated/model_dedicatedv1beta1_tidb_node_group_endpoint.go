...
type Dedicatedv1beta1TidbNodeGroupEndpoint struct {
	// The hostname or IP address of the endpoint. For `PRIVATE_ENDPOINT` connection types, availability varies by cloud provider:  - For clusters deployed on AWS, the `host` field is available after the private link service is ready.  - For clusters deployed on Google Cloud or Azure, the `host` field is available after you create a `PrivateEndpointConnection`.
	Host *string `json:"host,omitempty"`
	// The port of the endpoint connection.
	Port *int32 `json:"port,omitempty"`
	// The connection type for TiDB node group endpoints.  - `\"PUBLIC\"`: indicates that the endpoint is accessible over the public network.  - `\"VPC_PEERING\"`: indicates that the endpoint is accessible over [VPC peering](https://docs.pingcap.com/tidbcloud/tidb-cloud-glossary/#vpc-peering).  - `\"PRIVATE_ENDPOINT\"`: indicates that the endpoint is accessible over a private link.
