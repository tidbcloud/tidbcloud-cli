...
func (o ListOpts) RequiredFlags() []string {
	return []string{
		flag.ClusterID,
	}
}
...
func ListCmd(h *internal.Helper) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:    cobra.NoArgs,
		Example: fmt.Sprintf(`  List example resources (interactive):
  $ %[1]s serverless example list

  List example resources (non-interactive):
  $ %[1]s serverless example list -c <cluster-id>

  List example resources with json format:
  $ %[1]s serverless example list -o json`, config.CliName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// TODO implement the list logic, now just mock the payload
			...
			if format == output.JsonFormat || !h.IOStreams.CanPrompt {
				err := output.PrintJson(h.IOStreams.Out, payload)
				if err != nil {
					return errors.Trace(err)
				}
