...
func ListCmd(h *internal.Helper) *cobra.Command {
	...
	var listCmd = &cobra.Command{
		Use:         "list",
		Short:       "List all authorized networks",
		Args:        cobra.NoArgs,
		Aliases:     []string{"ls"},
		Annotations: make(map[string]string),
		Example: fmt.Sprintf(`  List all authorized networks in interactive mode:
  $ %[1]s serverless authorized-network list

  List all authorized networks in non-interactive mode:
  $ %[1]s serverless authorized-network list -c <cluster-id>

  List all authorized networks with json format:
  $ %[1]s serverless authorized-network list -o json`, config.CliName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			items, err := cloud.RetrieveAuthorizedNetworks(ctx, clusterID, d)
			if err != nil {
				return err
			}
			...
			if format == output.JsonFormat || !h.IOStreams.CanPrompt {
				err := output.PrintJson(h.IOStreams.Out, items)
				if err != nil {
					return errors.Trace(err)
				}
