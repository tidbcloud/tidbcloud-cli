...
func DescribeCmd(h *internal.Helper) *cobra.Command {
  ...
  $ %[1]s serverless private-link-connection describe -c <cluster-id> --private-link-connection-id <private-link-connection-id>`,
			config.CliName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			plc, err := d.GetPrivateLinkConnection(ctx, clusterID, plcID)
			if err != nil {
				return errors.Trace(err)
			}

			return errors.Trace(output.PrintJson(h.IOStreams.Out, plc))
		},
	}

	describeCmd.Flags().StringP(flag.ClusterID, flag.ClusterIDShort, "", "The cluster ID.")
	describeCmd.Flags().String(flag.PrivateLinkConnectionID, "", "The private link connection ID.")
