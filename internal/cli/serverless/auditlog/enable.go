...
func (c EnableOpts) NonInteractiveFlags() []string {
	return []string{
		flag.ClusterID,
	}
}

func (c *EnableOpts) MarkInteractive(cmd *cobra.Command) error {
...
func EnableCmd(h *internal.Helper) *cobra.Command {
  ...
  $ %[1]s serverless audit-log enable -c <cluster-id> `, config.CliName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(h.IOStreams.Out, color.GreenString(fmt.Sprintf("cluster %s database audit logging enabled", clusterID)))
			return nil
		},
	}

	enableCmd.Flags().StringP(flag.ClusterID, flag.ClusterIDShort, "", "The cluster ID.")
	return enableCmd
}
