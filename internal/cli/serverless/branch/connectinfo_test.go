...
func (suite *ConnectInfoSuite) TestConnectInfoArgs() {
	...
	branchBody := &branchModel.V1beta1Branch{}
	err = json.Unmarshal([]byte(getBranchResultStr), branchBody)
	assert.Nil(err)
	getClusterResult := &branchApi.BranchServiceGetBranchOK{
		Payload: branchBody,
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			stdoutString: "\nmysql -u 'yxfrrVaa55wvBKE.root' -h gateway01.us-east-1.dev.shared.aws.tidbcloud.com -P 4000 -D test --ssl-mode=VERIFY_IDENTITY --ssl-ca=/etc/ssl/cert.pem -p${password}\n",
		},
		{
