...
func (suite *DeletePrivateLinkConnectionSuite) TestDeletePrivateLinkConnectionArgs() {
	...
	suite.mockClient.On("DeletePrivateLinkConnection", ctx, clusterID, connectionID).Return(&privatelink.PrivateLinkConnection{}, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			args:         []string{"--cluster-id", clusterID, "--private-link-connection-id", connectionID, "--force"},
			stdoutString: fmt.Sprintf("Private link connection %s is deleted\n", connectionID),
		},
		{
